...
from utils.module_utils import TAGS_DUMP_FILE, CULLED_TRAIN_FILE, CULLED_TRAIN_FILE_SIZE, \
    DOCUMENTS_NO, TRAIN_FILE, print_overwrite

if __name__ == "__main__":
    with open(TAGS_DUMP_FILE, 'rb') as tags_dump_file:
        tag_list = pickle.load(tags_dump_file)['tag_list']

    tags_set = set([tag for count, tag in tag_list])

    #create a train file that has at least one example for each tag
    ...
    print "All tags will appear at least once in the first", written_rows, "documents"
